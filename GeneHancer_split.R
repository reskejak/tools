...
# prepare loop
iters <- ceiling(nrow(genehancer)/10000) # round up
last.iter <- nrow(genehancer) - ((iters-1)*10000)
...
for (n in 1:iters) {
  if (n < iters) {
    iter.n <- (1:10000)+((n-1)*10000) 
  } else {
    iter.n <- (1:(nrow(genehancer) - ((iters-1)*10000)))+((n-1)*10000)
  }
